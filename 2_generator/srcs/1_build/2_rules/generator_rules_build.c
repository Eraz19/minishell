...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   generator_rules_build.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gastesan <gastesan@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/08/04 16:40:08 by gastesan          #+#    #+#             */
/*   Updated: 2026/08/04 16:40:09 by gastesan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "generator_rules_priv.h"

static inline void	build_posix_rules_2(t_rule *rules)
{
...
static inline void	build_posix_rules(t_rule *rules)
{
	...
	build_posix_rules_2(rules);
}

static inline void	build_custom_rules(t_rule *rules)
{
	rules_build_accept(rules);
	rules_build_cmd_sub(rules);
}

void	rules_build(t_rule rules[RULE_COUNT])
{
	size_t	i;
	...
	while (i < RULE_COUNT)
		rules[i++] = (t_rule){0};
	build_posix_rules(rules);
	build_custom_rules(rules);
}
