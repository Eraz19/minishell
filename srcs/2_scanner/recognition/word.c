#include "scanner_recognition_.h"

static t_error	context_top_level_escape(t_lexer *lexer)
{
	t_escape_args	args;

	args.is_in_special_context = NULL;
	args.is_in_special_whitelist = NULL;
	args.enable_line_continuation = true;
	args.is_in_whitelist = is_in_context_none_whitelist;
	return (lexer_context_escape(lexer, args));
}

bool	is_in_middle_of_word(t_lexer *lexer)
{
	return (lexer->token->type == TOKEN_TOKEN);
}

t_error	lexer_rule_in_middle_of_word(t_lexer *lexer)  // Rule 8
{
	if (lexer->input->str.data[lexer->input->i] == '\\')
		return (context_top_level_escape(lexer));
	else
	{
		if (lexer->input->str.data[lexer->input->i] == '='
			&& lexer->token->assignment_offset == -1)
			lexer->token->assignment_offset = (ssize_t)lexer->token->value.len;
		return (lexer_consume(lexer, TOKEN_TOKEN, 1));
	}
}

t_error	lexer_rule_new_word(t_lexer *lexer) // Rule 10
{
	if (lexer->input->str.data[lexer->input->i] == '\\')
		return (context_top_level_escape(lexer));
	else
		return (lexer_consume(lexer, TOKEN_TOKEN, 1));
}
