#include "builtins.h"

#define NOT_UINT		"is not an unsigned decimal integer "
#define NOT_UINT_2		"greater than or equal to 1"
#define TOO_MANY_ARGS	"too many arguments"

static inline t_error	continue_too_many_args(
							const char *builtin_name,
							int *exit_status)
{
	t_error	err;

	err = error(ERR_INVALID_USAGE);
	err = error_print(err, builtin_name, TOO_MANY_ARGS, NULL, NULL);
	*exit_status = (int)err.type;
	err.type = ERR_POSIX_BUILTIN_SPECIAL;
	return (err);
}

static inline t_error	continue_invalid_arg(
							const char *builtin_name,
							const char *arg,
							int *exit_status)
{
	t_error	err;

	err = error(ERR_OPT_INVALID_ARG);
	err = error_print(err, builtin_name, arg, NOT_UINT NOT_UINT_2, NULL, NULL);
	*exit_status = (int)err.type;
	err.type = ERR_POSIX_BUILTIN_SPECIAL;
	return (err);
}

static inline t_error	continue_no_arg(t_runner *runner, int *exit_status)
{
	runner->control_depth = 1;
	*exit_status = 0;
	return (error(ERR_CONTINUE));
}

t_error	builtin_continue(int argc, char **argv, t_runner *runner, int *exit_status)
{
	int	first_operand_i;
	int	control_depth;

	first_operand_i = 1;
	if (argc > 1 && str_cmp(argv[1], "--") == 0)
		first_operand_i++;
	if (argc - first_operand_i > 1)
		return (continue_too_many_args(argv[0], exit_status));
	if (first_operand_i >= argc)
		return (continue_no_arg(runner, exit_status));
	if (parse_int(argv[first_operand_i], &control_depth) == false
		|| control_depth <= 0)
		return (continue_invalid_arg(argv[0], argv[first_operand_i], exit_status));
	runner->control_depth = (size_t)control_depth;
	*exit_status = 0;
	return (error(ERR_CONTINUE));
}
