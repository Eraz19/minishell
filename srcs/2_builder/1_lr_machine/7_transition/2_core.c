...
static bool	build_initial_lr_state(t_lr_machine *machine, t_lr_state *lr_states)
{
	...
	if (!lr_state_complete(machine, &lr_state))
		return (lr_state_free(&lr_state), false);
...
static bool	build_lr_state_and_transition(t_lr_machine *machine, t_lr_state *lr_state_from, t_symbol symbol)
{
	...
	if (!lr_state_next(machine, &lr_state_new, lr_state_from, symbol))
		return (false);
...
bool	transition_build_table(t_lr_machine *machine)
{
	...
	if (!build_initial_lr_state(machine, &machine->lr_states))
		return (false);
	i = 0;
