...
static inline t_error	sig_process_cmd(const t_string *cmd, int saved_status)
{
	...
	if (err.type == ERR_NO)
		err = params_set_last_status(saved_status);
	else if (err.type == ERR_EXIT)
		err = error_priorize(err, params_set_last_status(exit_status));
	else if (err.type == ERR_EXIT_WITH_CURRENT_STATUS)
	{
		err.type = ERR_EXIT;
		err = error_priorize(err, params_set_last_status(saved_status));
	}
	free(argv[0]);
	free(argv[1]);
	free(argv);
...
t_error	sig_process(void)
{
	...
	err = params_get_last_status(&saved_status);
	if (err.type)
		return (err);
