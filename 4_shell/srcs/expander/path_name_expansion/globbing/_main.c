...
static t_error	emit_path_matches(t_expander *expander, t_path_matches *matches)
{
	size_t			i;
	t_word_item_opt	opt;
	t_error			err;
...
static t_error	filter_existing_file(t_path_matches *matches)
{
	size_t			i;
	t_error			err;
	t_string		*match;
...
static t_error	find_path_comp_matches(
			t_path_matches *out,
...
static t_error	find_path_comps_matches(
					t_path_matches *out,
					const t_path_comps *comps)
{
	size_t			i;
	...
	{
		err = find_path_comp_matches(out, comps, i);
		if (err.type)
...
t_error	path_globbing(t_expander *expander, t_path_comps *src, bool *globbed)
{
	...
	*globbed = false;
	if (!path_comps_have_special(src))
		return (path_comps_free(src), expander->err = error(ERR_NO));
