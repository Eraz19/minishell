...
static inline bool	alias_is_valid_name(const char *name)
{
	size_t	i;
...
static inline t_error	alias_print(const char *name)
{
	if (name != NULL)
		return (env_print_one_alias(name));
	else
		return (env_print(ENV_PRINT_ALIAS));
}
