...
/**
 * @ingroup heredoc
 * @brief Scans @p body as one word with the here-document rules through
 *        @ref lexer_scan_word, then prefixes @c out->contexts with a
 *        @c CONTEXT_HEREDOC item spanning the whole body.
 *
 * @warning @p out is bound and initialized by the scan: on a failure
 *          before the scan runs it is left untouched (uninitialized).
 *
 * @param lexer Already initialized lexer (borrowed).
 * @param out Receives the scanned token; initialized by the scan. The
 *            caller releases it with @ref token_free (borrowed).
 * @param body Body text to scan; must be initialized by the caller; its
 *             text is moved into the lexer input stack (borrowed;
 *             ownership of the text taken by the input stack).
 ...
 */
t_error	scan_heredoc_body_context(t_lexer *lexer, t_token *out, t_string *body);
