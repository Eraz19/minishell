	...
	return ((t_token_recognition_context)
		{
			...
			.unescaped_args = NULL,
			.unescaped = param_context_unescape_rules,
			.escape = param_context_escape_rules,
			.is_end = is_context_param_ending,
		}
	);
}
...
t_error	scan_param_context(t_lexer *lexer)
{
	...
	return (scan_context(param_context_rules(lexer, item)));
}
