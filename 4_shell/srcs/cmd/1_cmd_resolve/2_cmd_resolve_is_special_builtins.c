...
static inline bool	cmd_resolve_is_special_builtin_2(t_cmd *cmd)
{
	...
	else if (str_cmp(cmd->name.data, "times") == 0)
		return (cmd_set_special_builtin(cmd, builtin_times), true);
...
bool	cmd_resolve_is_special_builtin(t_cmd *cmd)
{
	...
	else if (str_cmp(cmd->name.data, "exec") == 0)
		return (cmd_set_special_builtin(cmd, builtin_exec), true);
